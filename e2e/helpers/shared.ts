...
export function makeEnv(config: ApiConfig, appConfig: AppConfig) {
  const layers = LiveApiConfig(
    Config.struct({
      apiUrl: Config.succeed(config.apiUrl),
      headers: Config.succeed(config.headers),
    }),
  )
    ['|>'](Layer.provideMerge(HF.Client(fetch)))
    ['|>'](Layer.provideMerge(Layer.succeed(AppConfig, appConfig)))
    ...
    headers: cypressEnv('BASIC_AUTH_USER')
      ? Option.some(
          HashMap.make([
            'Authorization',
            toBase64(
              `${cypressEnv('BASIC_AUTH_USER')}:${cypressEnv(
                'BASIC_AUTH_PASSWORD',
              )}`,
            ),
          ]),
