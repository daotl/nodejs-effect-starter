...
>(
  ...
  const handler = makeRequestHandler<
    ...
    PR,
    RErr
  >(requestHandler, errorHandler, makeMiddlewareContext, (req) =>
    RequestContextContainer.flatMap((_) => {
      const r = makeUserProfileFromUserHeader(req.headers['x-user']).exit
        .runSync$
      ...
      return _.update(
        (_): RequestContext => ({
          ..._,
          userProfile: r.isSuccess() ? r.value : undefined,
        }),
      )
    }),
  )
