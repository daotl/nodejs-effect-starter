...
export function matchFor<Rsc extends Record<string, any>>(rsc: Rsc) {
  ...
  const matchWithServices: <Key extends keyof Rsc>(
    action: Key,
  ) => MatchWithServices<Key> = (action) => (services, f) =>
    matchWithServices_(action, services, f)
  ...
  const matchWith: <Key extends keyof Rsc>(action: Key) => MatchWith<Key> =
    (action) => (f) =>
      matchWith_(action, f)
  ...
  const r = {
    ...
    ...rsc.$$.keys.reduce(
      (prev, cur) => {
        ;(prev as any)[`match${cur as any}`] = Object.assign(
          matchWithServices(cur),
          {
            with: matchWith(cur),
