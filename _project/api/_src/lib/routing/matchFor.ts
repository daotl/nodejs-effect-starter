...
>(mod: TModules) {
  ...
  >(
    ...
    const handler = mod.$$.keys.reduce((prev, cur) => {
      prev[cur] = handle(mod[cur])(
        handlers[cur as keyof typeof handlers] as any,
      )
      return prev
    }, {} as any)
...
export function matchFor<Rsc extends Record<string, any>>(rsc: Rsc) {
    ...
    matchAction(
      ...
      Effect.context<Values<SVC>>().flatMap((context) =>
        allLowerFirstWith_(
          services,
          (svc) => (req: any, ctx: any) =>
            Effect.context<Values<SVC2>>().flatMap((context2) =>
              allLowerFirst_(fctx(req, { ...ctx, ...(svc as any) })).flatMap(
                (svc2) =>
                  f(req, {
                    ...ctx,
                    ...(svc as any),
    ...
    return (...args: any[]): any => {
      if (args.length === 1) {
        return typeof args[0] === 'function'
          ? matchWith_(action, args[0])
          : matchWithEffect_(action, args[0])
      } else if (args.length === 2) {
        return matchWithServices_(action, ...(args as [any, any]))
      } else {
        return matchWithServices2_(
          action,
          args[0],
  ...
  type MatchWithEffect<Key extends keyof Rsc> = <
    R,
    R2,
    E,
    E2 extends SupportedErrors,
  ...
  type MatchFn<Key extends keyof Rsc> = MatchWithServices<Key> &
