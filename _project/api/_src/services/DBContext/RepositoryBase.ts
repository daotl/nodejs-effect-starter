...
import type {
  ...
  StoreConfig,
  Where,
} from '@effect-app/infra/services/Store'
...
export const RepositoryBase = <Service>() => {
  ...
  >(
    ...
    abstract class RepositoryBaseC implements Repository<T, PM, Evt, ItemType> {
      ...
      abstract find: (
        id: T['id'],
      ) => Effect<ContextMap | RequestContextContainer, never, Opt<T>>
      abstract all: Effect<ContextMap, never, Chunk<T>>
      abstract saveAndPublish: (
        items: Iterable<T>,
        events?: Iterable<Evt>,
      ) => Effect<
        ContextMap | RequestContextContainer,
      ...
      abstract utils: {
        ...
        ) => Effect<ContextMap, never, Chunk<PM>>
      }
    ...
    function make(
      ...
      return Do(($) => {
        ...
        const all = allE.flatMap((_) =>
          _.forEachEffect(EParserFor(schema).condemnDie),
        )
        ...
        const saveAll = (a: Iterable<T>) =>
          saveAllE(a.toChunk.map(Encoder.for(schema)))

        const saveAndPublish = (items: Iterable<T>, _: Iterable<Evt> = []) =>
          saveAll(items) > publishEvents(_)
        ...
        const r: Repository<T, PM, Evt, ItemType> = {
          ...
          utils: {
            ...
            filter: store.filter.flow((_) =>
              _.tap((items) =>
                ContextMap.access.map(({ set }) =>
                  items.forEach((_) => set(_.id, _._etag)),
                ),
              ),
            ),
            all: store.all.tap((items) =>
              ContextMap.access.map(({ set }) =>
                items.forEach((_) => set(_.id, _._etag)),
              ),
            ),
...
>() {
  ...
  function where(
    makeWhere: (f: WhereFilter) => Where | readonly [Where, ...Where[]],
    mode?: 'or' | 'and',
  ) {
...
export const RepositoryBaseImpl = <Service>() => {
  return <
    ...
    >(
      ...
      const encode = Encoder.for(schema)
      return class extends RepositoryBase<Service>()<T, PM, Evt, ItemType>(
        itemType,
      ) {
        constructor(
        ...
        private findE(id: T['id']) {
          return this.store.find(id).flatMap((items) =>
            Do(($) => {
              const { set } = $(ContextMap.access)
        ...
        override find = (id: T['id']) =>
          this.findE(id).flatMapOpt(EParserFor(schema).condemnDie)
        ...
        override all = this.allE.flatMap((_) =>
          _.forEachEffect(EParserFor(schema).condemnDie),
        )
        private saveAllE = (a: Iterable<E>) =>
          Effect(a.toNonEmptyArray).flatMapOpt((a) =>
            Do(($) => {
              const { get, set } = $(ContextMap.access)
              const items = a.mapNonEmpty((_) =>
                this.mapToPersistenceModel(_, get),
              )
        ...
        private saveAll = (a: Iterable<T>) =>
          this.saveAllE(a.toChunk.map(Encoder.for(schema)))

        override saveAndPublish = (items: Iterable<T>, _: Iterable<Evt> = []) =>
          this.saveAll(items) > this.publishEvents(_)
        ...
        override utils = {
          ...
          parse: Parser.for(schema).unsafe,
          filter: this.store.filter.flow((_) =>
            _.tap((items) =>
              ContextMap.access.map(({ set }) =>
                items.forEach((_) => set(_.id, _._etag)),
              ),
